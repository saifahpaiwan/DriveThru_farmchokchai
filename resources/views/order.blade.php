                                ...
                                @if(count($data['result_order'])>0)
                                    @foreach($data['result_order'] as $row) 
                                        <a href="{{ url('/orderviwe/'.$row->orders_id) }}">
                                            <div class="d-flex text-left pb-3 pt-3" style="border-bottom: 1px solid #ddd;"> 
                                                <div> 
                                                    เลขใบสั่งซื้อ : {{$row->order_code}}<br>
                                                    ผ่านมา : {{Carbon\Carbon::parse($row->created_at)->diffForHumans()}}
                                                </div>
