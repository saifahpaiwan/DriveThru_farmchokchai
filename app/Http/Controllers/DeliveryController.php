...
class DeliveryController extends Controller
{
    ...
    */
    ...
    public function resultOrder($orderId){
        ...
        if($orderId){
            ...
            foreach($Query as $row){   
                ...
                $items["delivery_form"]        = ($row->delivery_form==1)? "โอนเงินผ่านธนาคาร" : "ชำระด้วยเงินสด"; 
    ...
    public function Querysubs()
    {
        ...
        ->where('product_subs.category_id', 8)
        ->get();
        return $data;
    }
    ...
    {    
        ...
        if(isset($request->delivery_form)){
            ...
            } else if($request->delivery_form==2){
                $delivery_form="ชำระด้วยเงินสด";
                $validatedData = $request->validate(
                    [
                    'payPrice' => 'required',  
                    ],
                    [ 
                        'payPrice.required'=>' กรุณาระบุจำนวนเงินของท่านที่จ่ายจริง!',   
                    ]
                );
            }
        }
        ...
        $msg_total=number_format(($total_price)-$discount, 2);
        ...
        $message.="รายละเอียดรถ : ".$user->additional_address." \n"; 
        if($request->delivery_form==1){
            $message.="การชำระเงิน : ".$delivery_form." \n"; 
        } else if($request->delivery_form==2){
            $message.="การชำระเงิน : ".$delivery_form." \n"; 
            $message.="จำนวนที่จ่าย : ".number_format($moreDetails,2)." ฿ \n"; 
        }   
    ...
    function notify_message($message){
        ...
        // $token = "3LJojycplYHWjLYx2R9D2ZpbKhWMTEb6eUXpoMNpRyX";  
        ...
        $accToken = "3LJojycplYHWjLYx2R9D2ZpbKhWMTEb6eUXpoMNpRyX";
    ...
    {  
        $validatedData = $request->validate([
            ...
            'carDetail4' => 'required|string|max:100', 
            'payType' => 'required', 
        ]);
        $id=Auth::user()->id; 
        $carDetail1=""; $session_zone="";  
        if(isset($request->zone)){
            $session_zone=$request->zone;  
        }

        if(isset($request->carDetail1)){
            $carDetail=$request->carDetail1.",".$request->carDetail2.",".$request->carDetail3.",".$request->carDetail4.",".$session_zone;
        }
        $data=array(
            'sender_fname'=>$request->fname,  
            'sender_phone'=>$request->tel, 
            'additional_address'=>$carDetail,
        );
        DB::table('users')->where('id', $id)->update($data); 
        $deliveryForm = session()->get('deliveryForm');   
        $deliveryForm = $request->payType;
